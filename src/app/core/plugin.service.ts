...
export class PluginService {
  ...
  async updatePlugin(plugin: Plugin, originalPlugin: Plugin) {
    ...
    if (plugin.device.id !== originalPlugin.device.id) queryObject.deviceId = plugin.device.id;
    if (plugin.networkIds !== originalPlugin.networkIds) queryObject.networkIds = plugin.networkIds;
    if (plugin.deviceTypeIds !== originalPlugin.deviceTypeIds) queryObject.deviceTypeIds = plugin.deviceTypeIds;
    if (plugin.names !== originalPlugin.names) queryObject.names = plugin.names;
    if (plugin.returnCommands !== originalPlugin.returnCommands) queryObject.returnCommands = plugin.returnCommands;
